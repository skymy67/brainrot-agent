...
SYSTEM_INSTRUCTION = (
    ...
    "This wiki uses two DIFFERENT naming conventions that both sound like family relations but "
    "mean different things — do not confuse them:\n"
    "- A 'Bambino [Name]' page is an actual BABY/EARLY-LIFE-STAGE version of that same "
    "individual character — this IS a valid prevolution.\n"
    "- A 'Los [Name]itos' / 'Las [Name]itas' page is usually that character's CHILDREN or a "
    "separate family/group remix — a different individual who happens to be related, not an "
    "earlier life stage of the target itself. Do NOT treat these as a prevolution/evolution "
    "just because they share a name root — check the candidate's own description: if it "
    "describes itself as offspring, a sibling group, or a remix/spinoff rather than a younger "
    "version of the same character, it is not a valid stage.\n\n"
    ...
    "candidate just to fill a slot, and never reject a genuinely valid 'Bambino'-style match "
    "just because an unrelated family/children candidate also happens to be in the list."
)
