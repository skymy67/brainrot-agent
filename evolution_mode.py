...
SYSTEM_INSTRUCTION = (
    ...
    "not the same base just because one is themed around the other's food). If locked as "
    "stage1, only select from evolutions (things that come after); if locked as final, only "
    "select prevolutions (things that come before); if locked as middle, select at most one of "
    "each. Select at most two stages on any one side. If no candidate plausibly fits a side, "
    "leave that list empty rather than forcing a weak match — never pick a candidate just to "
    "fill a slot."
)
